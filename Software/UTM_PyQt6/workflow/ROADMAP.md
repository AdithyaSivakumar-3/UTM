...
- 🟡 **SF20 Manual blob selection** (Mirza's suggestion, built 2026-09-05): a Select Blobs button (both camera rows, before Calibrate Px₀) opens a frozen-frame picker; the operator clicks the two markers and `detect_blobs` then tracks the component NEAREST each pick — no area gate, no circularity gate, no pair chooser. Seeds follow the markers frame to frame, so one pick survives the whole pull; the separation and speed guards still apply, and the feed caption says `[MANUAL blobs]` while the gates are off. Default is auto — the switch lives in Settings ▸ DIC camera setup ▸ Blob selection, session-only (seeds belong to a mounting). 9 offline tests on real S13 frames (`tests script/test_manual_blobs.py`), incl. the defining case: MIN_CIRCULARITY 0.995 kills auto, manual keeps tracking the same centroids to <2 px. 🟦 built + offline-verified; **needs a rig session** (pick, pull, fracture) before carding in the deck.
...
> 7. **✅ SF13 — guided wizard. BUILT 2026-08-18.** `Software/UTM_PyQt6/app/utm_wizard.py` +
...
>    runs SF1–SF17 (**16 rig-validated · 2 built · 1 hardware-blocked**), but five features have no
...
>    ▸ **TWO OVERVIEW SLIDES, ~10 cards each** — SF1–SF10 and SF11–SF17 — so the whole feature set
...
>
>    ▸ **SF17's frames need no operator time** — one still from each of the three AVI styles
>    (raw / boost / speckle) can be pulled straight out of the S25/S26/S13 captures already on disk.
>    ▸ **SF17 is the subtle one.** p219–224 already exists but is a DATA-VALIDATION slide
...
- ✅ **POSTERS — BUILT 2026-08-12, THREE EDITIONS from one content spec.**
  ...
  - **Features run in NUMERIC order SF1 → SF15 on every edition**, as a full-width band UNDER the
  ...
  - **SF14/16 added to the registry below** — the test registry and the dead-DIC guard + hard backstops
...
- ~~⬜ RECORD THE FRACTURE TEST ON VIDEO — for independent analysis in the extensometer software at MOT.~~
  ...
  **Done =** a fracture run whose footage loads in MOT's software, and a plot of their strain vs ours
  ...
  same material).
  *(If this gets carded as a feature it becomes **SF16** — numbering is append-only, see §3c.)*
...
| **19** | **Noise capture** — records the instrument AT REST for a chosen window and separates offset and drift (correctable out of a later run) from the residual sd (the measurement uncertainty, which does NOT subtract). Deck p309 | 🟦 built, verified offline, not yet run on the rig |
| **20** | **Manual blob selection** — operator picks the two markers on a frozen frame; tracking follows the picks with the shape/size gates OFF (pair guards stay on). Overrides auto detection; default remains auto | 🟦 built, verified offline (9 tests), not yet run on the rig |

As of 2026-09-02: **19 features are carded** — SF1–SF13, SF14, SF15, SF16–SF19. Eighteen are built
and rig-validated; **SF19** is built and verified offline but has not yet recorded a real pull.
**RENUMBERED 2026-09-02 to a contiguous SF1–SF19.** Two entries were dropped, by the test that a
card has to be something the OPERATOR invokes or that acts on the machine during a run:

- **Measured Poisson / true Cauchy** (optics-blocked, never built) — the work is **FW1** in §6.
- **Simulate-first harness** (a DEVELOPER practice: `control_sim` is imported 0 times by `main.py`
  and has no serial or camera path, so an operator can never invoke it). It lives on as a METHODS
  bullet on the posters — "Simulated before hardware" — which is the honest place for it.

The list was then closed up, so **the earlier rule that numbers are append-only and never reused no
longer holds**. It held while the numbering had gaps; it was dropped deliberately, because a card
list shown to an audience should not have holes in it. Anything printed before this date uses the
OLD numbers, so:

| printed before 2026-09-02 | now |
|---|---|
| SF1 – SF13 | unchanged |
| SF14 Multi-marker Poisson | dropped → FW1 |
| SF15 Test registry | **SF14** |
| SF16 Dead-DIC guard + backstops | **SF15** |
| SF17 Simulate-first harness | dropped |
| SF18 Live Px₀ overlay | **SF16** |
| SF19 Video + image capture | **SF17** |
| SF20 DIC post-processing | **SF18** |
| SF21 Noise capture | **SF19** |

The two dropped entries, kept verbatim for the record:

| ~~14~~ | ~~Multi-marker Poisson / true Cauchy~~ — **RETIRED 2026-09-02**, by the same test that retired SF17: a card has to be something the OPERATOR invokes or that acts on the machine during a run. This is optics-blocked and was never built, so the card described an intention rather than a feature. The work is not dropped — it is **FW1** in §6, and the slides explaining how Poisson would be measured stay. The number is burnt, not reused. | — retired |
...
⚠️ **SF14–SF17 were added 2026-08-12** while building the poster — all three were built and in daily use but had never been carded. **SF17 was then retired the same day**: a card has to be something the OPERATOR uses on the rig, and a developer-only simulation harness is not that. Test for admitting a feature: *can the person running a test invoke it, or does it act on the machine during a run?* If not, it belongs in METHODS, not in the feature registry.

✅ **`utm_registry` (SF14) IS wired into `main.py` — this warning is stale, corrected 2026-09-01.** `_sf11_after_save` calls `utm_registry.add` on every save, gated by Settings ▸ 'On save: add to the test registry', which defaults ON and is remembered. It remains a CLI as well. Original note: it is a CLI you run (`scan`/`list`/`add`). It stays carded because it is operator/analyst-facing and produces a real artifact, but do NOT describe it as automatic; the automation is SF11, still planned.
...
- 🟡 **Phase D — UX layer.** The **wizard is DONE** (SF13, `utm_wizard.py`, View ▸ Guided wizard). What is left is the **live analysis overlay** (live E / predicted UTS / fracture flag) and the **glanceable dashboard + audio cue on fracture; event-annotation hotkey.
- 🟡 **DIC auto-calibrate (Phase C remainder).** The **sweep is DONE** (`utm_autocal.py`, Settings ▸ DIC camera setup ▸ Auto-calibrate) and since 2026-09-01 its result **survives a restart**. What is left is **auto-follow ROI** — shifting the offset to keep markers centred through a ductile draw. Blocked on hardware, not effort: `set_roi`'s own note says Width/OffsetX cannot be changed on a streaming Basler, so whether the offset can move mid-acquisition has to be established ON the camera. It also runs during a destructive pull, so it cannot ship unverified — a wrong crop mid-test costs a specimen. Note `frame_headroom()` already answers the SETUP half (where to aim the crop for a target strain) and warns before the pull.
- 🟡 **Auto-metadata + foldering** and **one-click per-specimen deck**. **Auto-metadata is DONE** (SF11: capture link, registry row, optional report, all on save). **Foldering is DONE 2026-09-01** — Save proposes `Specimen_S<n>/` and warns when the path will give the registry row no specimen id. What is left is the **one-click per-specimen deck** (extract the pptx builders → `utm_slides.py`). Worth scoping small: a per-specimen deck needs a title, the load/time and stress/strain figures and the key numbers, all of which `utm_analysis` and `utm_report` already produce — it does NOT need the 166-slide V6a builders refactored, and doing that carries real risk to a deck that is the research record.
...
- ✅ **`main.py` snapshot commit — CLOSED 2026-09-01.** `main.py` is now committed with every change rather than between named snapshots; the working tree is clean and `ui/utm_mainwindow.ui` is tracked. Original note:
