...
def _name(B, k):
    return "XT205-S2 · MOT" if k == "MOT" else "%s · PPD-UTM DIC" % k
...
def fig_stress(B=None):
    ...
    ax.set_title("Stress–strain: MOT XT-205 against the PPD-UTM DIC's 45 mm runs",
...
def fig_pair(B=None):
    ...
    ax.set_title("XT205-S2 against S34 — the two-specimen comparison, both PLA",
