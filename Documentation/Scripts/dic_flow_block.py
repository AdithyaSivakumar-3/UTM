...
# ================================================ 2b. the same pipeline when the pair is picked
# by hand (SF20). Mirrors the auto slide box for box so the diff IS the content: the front end
# and the guards are the same code; only "which blobs" is answered differently.
s = prs.slides.add_slide(BLANK); ju(s)
title(s, "INSIDE detect_blobs — WHEN THE PAIR IS PICKED BY HAND (SF20)")

_x, _w = 0.42, 3.05
_col2, _col3 = 4.62, 8.82
_y0, _h, _gap = 1.10, 0.66, 0.30

# ---- column 1: identical front end — the operator's picks change nothing here
_c1 = [("① the frame arrives\n2348 × 419, grey 0–255  — SAME", _GREY_DOM, _GREY_EDGE),
       ("② mask_x — blank the columns\noutside the specimen  — SAME", _GREY_DOM, _GREY_EDGE),
       ("③ cv2.threshold — SAME cut\n(fixed by default, Otsu if enabled)", _CUT, _CUT_EDGE),
       ("④ binary image — SAME\nevery pixel 0 or 255", _BIN_DOM, _BIN_EDGE)]
for i, (t_, f, e) in enumerate(_c1):
    y = _y0 + i * (_h + _gap)
    flow(s, _x, y, _w, _h, t_, fill=f, border=e, fs=9.0)
    if i < len(_c1) - 1:
...
banner(s, _x, _y0 + 4 * (_h + _gap) - 0.06, _w, 0.72,
       "STEPS ① – ⑤ ARE THE AUTO PIPELINE, UNCHANGED. Manual mode never touches how pixels "
       "become candidates — only which candidates are taken.",
       fill=_BIN_DOM, fg=BLACK, fs=9.0)

# ---- column 2: what replaces the gates and the chooser
_c2 = [("⑤ findContours(binary,\nRETR_EXTERNAL)  — SAME", _BIN_DOM, _BIN_EDGE),
       ("⑥ per contour → centroid.\nOnly floor: area ≥ 20 px²\n(sensor noise, not a gate)",
        _BIN_DOM, _BIN_EDGE),
       ("⑦ YOUR TWO PICKS ARE SEEDS\neach takes the NEAREST component\nwithin 150 px — "
        "shape/size gates OFF", YELLOW_WARN, RGBColor(0xD2, 0xA6, 0x1B)),
       ("⑧ one component per seed\nmerged or vanished ⇒ honest dropout,\nnever a fabricated pair",
        YELLOW_WARN, RGBColor(0xD2, 0xA6, 0x1B))]
for i, (t_, f, e) in enumerate(_c2):
    y = _y0 + i * (_h + _gap)
    flow(s, _col2, y, _w, _h, t_, fill=f, border=e, fs=8.8)
    if i < len(_c2) - 1:
...
flow(s, _col2, _y0 + 4 * (_h + _gap) - 0.06, _w, 0.72,
     "no find within 150 px → the seed KEEPS its\nposition — a one-frame flicker cannot strand it",
     fill=YELLOW_WARN, border=RGBColor(0xD2, 0xA6, 0x1B), fs=8.6)

# ---- column 3: the pick follows, and every guard stays
_c3 = [("⑨ the found centroid becomes\nNEXT frame's seed — the pick\nFOLLOWS the marker",
        YELLOW_WARN, RGBColor(0xD2, 0xA6, 0x1B)),
       ("⑩ sort the two by Y\ntop marker first  — SAME", _BIN_DOM, _BIN_EDGE),
       ("⑪ plausibility check — SAME\nseparation vs Px₀, and the\nspeed guard", _BIN_DOM,
        _BIN_EDGE),
       ("⑫ two centroids\n→ calculate_dic_strain  — SAME", GREEN_PASS, DARK_GREEN)]
for i, (t_, f, e) in enumerate(_c3):
    y = _y0 + i * (_h + _gap)
    flow(s, _col3, y, _w, _h, t_, fill=f, border=e, fs=8.6)
    if i < len(_c3) - 1:
...
tb(s, _col3, _y0 + 4 * (_h + _gap) - 0.06, _w, 0.80,
   "150 px search: far above frame-to-frame motion, far below the 940–1680 px pair separation "
   "— a seed cannot wander onto the OTHER marker; a marker that LEFT is lost, not replaced.",
   fs=8.2, italic=True, colour=GREY_TEXT)

banner(s, 0.42, 5.62, 12.55, 0.52,
       "EVERYTHING ELSE IS SHARED. Threshold, contours, Px₀, the pair and speed guards, the "
       "badge and the strain maths are the same code path — manual mode only answers "
       "“WHICH blobs?” with the operator's picks instead of shape and size.",
       fill=GREEN_PASS, fg=DARK_GREEN, fs=10.5)

tb(s, 0.42, 6.24, 12.55, 0.62,
   "Why it exists (SF20): dots the gates keep rejecting — overspray crescents fused to a rim, a "
   "torn dot, an unusual specimen. The operator has SAID which two things are the markers, so "
   "shape and size stop being evidence. Auto stays the default; without seeds, manual mode "
   "simply keeps running the auto pipeline.",
   fs=9.4, colour=BLACK)
