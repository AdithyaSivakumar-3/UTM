"""Replace NAMED Weekly slides in place with their rebuilt V6a versions. Nothing else.

    python wk_replace_pairs.py <v6a.pptx> <weekly.pptx> 340:190 341:191

WHY THIS EXISTS. When only a slide's TEXT changes it can be edited in place. When its FIGURE
changes it cannot — the old image stays, and repointing the image alone leaves it at the previous
aspect inside a box sized for it. The lazy answer is to drop the whole block and re-append it,
which is what was done on 2026-09-03 and is wrong: it rewrites slides nobody asked to change, and
any hand edit on one of them is gone without trace.

So: one slide out, one slide in, at the same position, for the slides named on the command line
and no others.

THE GUARANTEE IS CHECKED, NOT ASSERTED. Every slide is fingerprinted (shape geometry + all text +
all table cells) before the edit and again after. Any slide that is not a named target and is not
byte-identical afterwards aborts the run before it is saved. That covers the failure this script
exists to prevent — a replacement quietly shifting or touching a neighbour.

Positions are handled through the sldId ELEMENTS rather than indices: the new slide is appended,
moved to where the old one sat, and the old one removed with its relationship. Index arithmetic
across several swaps is exactly how a deck gets silently reordered.
"""
import copy
import hashlib
import io
...
if not PAIRS:
    sys.exit("give at least one  weeklyIndex:v6aIndex  pair")
...
def title_of(slide, house):
    """The TOPMOST wide text box — never the longest, which on a Weekly slide is a paragraph."""
    best = None
    for sh in slide.shapes:
...
def main():
    ...
    targets = sorted(w for w, _v in PAIRS)
    for w, v in PAIRS:
        if not (1 <= w <= before_n):
            sys.exit("Weekly slide %d does not exist" % w)
        if not (1 <= v <= len(ss)):
            sys.exit("V6a slide %d does not exist" % v)
        print("Weekly %d   %r" % (w, title_of(ds[w - 1], True)[:66]))
        print("      <-   V6a %d   %r" % (v, title_of(ss[v - 1], False)[:66]))

    olds = [list(lst)[w - 1] for w, _v in PAIRS]
    layout = next(l for l in dst.slide_layouts if l.name == LAYOUT)
    sldnum, house = furniture(ds)

    news = []
    for _w, v in PAIRS:
        bring(dst, layout, ss[v - 1], sldnum, house)
        news.append(list(lst)[-1])          # add_slide appends; the new element is last

    print()
    for (w, _v), new_el, old_el in zip(PAIRS, news, olds):
        pos = list(lst).index(old_el)
        lst.remove(new_el)
        lst.insert(pos, new_el)
        rid = old_el.get("{%s}id" % R_NS)
        lst.remove(old_el)
        try:
            dst.part.drop_rel(rid)
        except Exception:
            pass
        print("   Weekly %d replaced in place" % (pos + 1))

    # ---- page references on the replaced slides only
    now = list(dst.slides)
    titles = {}
    ...
    def target(page):
        ...
        if not (0 <= j < len(ss)):
            return None, "page %d outside the source deck" % page
        key = title_of(ss[j], False).strip().lower()
        if not key:
            return None, "source slide has no title"
        if key in titles:
            return titles[key], key
        hits = sorted({x for k, x in titles.items() if key in k or k in key})
        return (hits[0], key) if len(hits) == 1 else (None, "%d matches" % len(hits))

    print()
    for w, _v in PAIRS:
        for sh in now[w - 1].shapes:
            ...
            for para in sh.text_frame.paragraphs:
                for run in para.runs:
                    ...
                    for m in PAGE_RE.finditer(run.text):
                        pg, why = target(int(m.group(1)))
                        if pg is None:
                            print("   ref %s LEFT ALONE — %s" % (m.group(0), why))
                            continue
                        out = out.replace(m.group(0), "p%d" % pg)
                        print("   ref %s -> p%d   (%s)" % (m.group(0), pg, why[:52]))
                    run.text = out

    # ---- THE GUARANTEE, checked before anything is written
    after_fp = [fingerprint(s) for s in list(dst.slides)]
    if len(after_fp) != before_n:
        sys.exit("ABORTED — slide count changed from %d to %d" % (before_n, len(after_fp)))
    collateral = [i + 1 for i in range(before_n)
                  if (i + 1) not in targets and before_fp[i] != after_fp[i]]
    if collateral:
        sys.exit("ABORTED — these slides changed and were not asked to: %s" % collateral)
    print("\nverified: %d slide(s) replaced, %d others byte-identical"
          % (len(targets), before_n - len(targets)))

    backup = DST.replace(".pptx", " (before swap %s).pptx" % datetime.now().strftime("%H%M"))
    ...
    if n != before_n:
        raise OSError("expected %d slides, package has %d" % (before_n, n))
    os.replace(part, DST)
    os.remove(backup)
