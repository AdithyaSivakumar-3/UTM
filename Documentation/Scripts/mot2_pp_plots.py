...
def fig_full(S=None):
    ...
    a1.set_title("DIC strain vs time — the whole pull, both calculations, XT205-S2 to fracture",
                 fontsize=10.5, color=INK)
    ...
    # name the three lines, so grey is never mistaken for data: grey is the ZERO of this panel
    a2.text(58, 7, "grey = zero — where the two calculations would be identical",
            fontsize=7.6, color="#8A8A8A", va="bottom",
            bbox=dict(boxstyle="round,pad=0.18", fc="white", ec="none", alpha=0.8))
    a2.text(0.012, 0.06, "blue = the per-frame difference, PPD-UTM DIC − XT-205",
            transform=a2.transAxes, ha="left", va="bottom", fontsize=7.6, color=C_PP,
            bbox=dict(boxstyle="round,pad=0.18", fc="white", ec="none", alpha=0.8))
    a2.text(0.985, 0.90, "black dashes = the constant %.0f µε — %.3f px on a %.0f px span"
            % (D["offset_ue"], D["offset_px"], D["px0"]),
            transform=a2.transAxes, ha="right", va="top", fontsize=8.2, color=INK,
            fontweight="bold")
    a2.set_xlabel("time from the preload (s)", fontsize=9.5)
    # "PPD-UTM DIC − XT-205 (µε)" reads like a noise trace and was taken for one. It is a
    # DIFFERENCE, and a difference cannot say whose noise is larger: subtracting two records
    # ADDS their scatter.
    a2.set_ylabel("difference, µε\n(PPD-UTM DIC − XT-205)", fontsize=9)
