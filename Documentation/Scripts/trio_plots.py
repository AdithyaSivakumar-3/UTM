...
sys.path.insert(0, os.path.join(ROOT, "Software", "UTM_PyQt6", "app"))
...
#   PLA   — Chacón et al. 2017 + Tymrak et al. 2014; Ultimaker/Prusament PLA TDS inside
#   PETG  — Durgashyam et al. 2019 + Hsueh et al. 2021; Prusament PETG TDS inside
#   (UTS bands aligned 2026-09-06 with petg_data.LIT — one band everywhere)
#   TPU   — TPU 95A TDS (Ultimaker, SainSmart, NinjaFlex); Hohimer et al. 2020
LIT = {
    "PLA":  {"E": (2000, 3500), "sig": (32, 60), "eps": (2, 8)},
    "PETG": {"E": (1500, 2100), "sig": (30, 50), "eps": (5, 25)},
...
def fig_curves(out="trio_curves.png"):
    ...
    # UTS of the two stiff materials, marked with values (computed from the curves, not typed);
    # PLA's label goes in the empty upper-left, PETG's below its own falling branch — both
    # clear of the title, the legend and each other.
    for m, tx, ty in (("PLA", 4.6, 41.8), ("PETG", 4.7, 31.5)):
        e, sarr, _ = curve(REP[m])
        k = int(np.argmax(sarr))
        ax.plot([e[k]], [sarr[k]], "o", ms=8, color=COL[m], mec="white", mew=1.3, zorder=6)
        ax.annotate("UTS  %.1f MPa\nat %.1f %% strain" % (sarr[k], e[k]),
                    xy=(e[k], sarr[k]), xytext=(tx, ty), fontsize=8.8,
                    color=COL[m], fontweight="bold",
                    arrowprops=dict(arrowstyle="->", color=COL[m], lw=1.1))
